...
export default function Nav() {
  ...
  const [indicator, setIndicator] = useState<{ top: number; left: number }>({
    top: 0,
    left: 0,
  });
  ...
  const measure = useCallback(() => {
    ...
    const a = linkRefs.current[activeIndex];
    if (!ul || !a) return;
    setIndicator({
      top: a.offsetTop + a.offsetHeight / 2,
      left: a.offsetLeft - 8 - 22,
    });
  }, [activeIndex]);
    ...
    <nav className={styles.nav} aria-label="Primary">
      ...
      <ul className={styles.social} aria-label="Social">
        <li>
          ...
          >
            <svg
              className={styles.iconSoundcloud}
              viewBox="0 -3.5 20 16"
              fill="none"
              aria-hidden="true"
              focusable="false"
