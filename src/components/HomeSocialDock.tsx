...
import styles from "./HomeSocialDock.module.css";

/**
 * Bottom-right social dock (Figma 289-1213). The three social icons that used to
 * live in the top-right menu now sit in the bottom-right corner — anchored 64px
 * from the right + bottom edges (--corner-gap), the same corner the release
 * previewer used (which is retired on these routes). Desktop-only, shown on the
 * Music routes listed in socialDockRoutes (Home first; RELEASES / A DAY WITH /
 * ABOUT now match it), so nothing else changes.
 *
 * Above the WebGL canvas (--z-menu) so the links are clickable; same links as before
 * (Instagram stays 2h2h_studio) opening in a new tab.
 */
...
export default function HomeSocialDock() {
  // Desktop social-dock routes only (Home / RELEASES / A DAY WITH / ABOUT); other
  // routes keep their existing top-right social. Mobile is CSS-hidden below.
  if (!hasSocialDock(usePathname())) return null;

  return (
    <ul className={styles.dock} aria-label="Social">
      <li>
        <a
        ...
        >
          <svg className={styles.iconSoundcloud} viewBox="0 -3.5 20 16" fill="none" aria-hidden="true" focusable="false">
