...
/**
 * The mobile burger + full-viewport menu overlay, shared by the MUSIC site (dark) and
...
export default function MobileMenu({ theme = "dark" }: { theme?: "dark" | "light" }) {
  ...
  // Auto-close on route change (covers browser Back/Forward).
  ...
  // Body scroll lock + Escape + focus trap while open; restore focus to the burger.
  useEffect(() => {
    if (!open) return;
      ...
      >
        <svg viewBox="0 0 32 32" width="32" height="32" aria-hidden="true">
          {open ? (
            ...
            <path d="M6 11 H26 M6 21 H26" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
          )}
        </svg>
      </button>
